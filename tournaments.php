...
if (isset($_POST['join_tournament'])) {
    ...
    if (!$tournament) {
    ...
    } elseif (strtotime($tournament['start_time']) < time()) {
        $response = "Tournament registration is closed.";
        ...
        if ($exists && mysqli_num_rows($exists) > 0) {
            ...
            $availableBalance = (float) $wallet['balance'] + (float) $wallet['winnings_balance'];
            if ($availableBalance < $entryFee) {
                $response = "Insufficient balance.";
            } else {
                $feeSafe = number_format($entryFee, 2, '.', '');
                $useWinnings = min((float) $wallet['winnings_balance'], $entryFee);
                $useWallet = $entryFee - $useWinnings;
                ...
                $updateBalance = mysqli_query($con, \"UPDATE user_wallets SET balance = balance - $useWallet, winnings_balance = winnings_balance - $useWinnings WHERE user_id={$user['id']}\");
...
$tournaments = mysqli_query($con, "SELECT * FROM tournaments WHERE status!='ended' AND start_time >= NOW() ORDER BY start_time ASC");
?>
<!DOCTYPE html>
<html lang="en">
