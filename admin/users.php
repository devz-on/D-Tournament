...
$usersQuery = "SELECT u.*, w.balance, w.winnings_balance FROM users u LEFT JOIN user_wallets w ON w.user_id = u.id";
if ($searchQuery !== '') {
    $usersQuery .= " WHERE u.username LIKE '%$searchQuery%' OR u.email LIKE '%$searchQuery%'";
}
$usersQuery .= " ORDER BY u.created_at DESC";
$users = mysqli_query($con, $usersQuery);
                                    ...
                                    <?php if ($users && mysqli_num_rows($users) > 0) { ?>
                                        <?php while ($row = mysqli_fetch_assoc($users)) { ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($row['email_verified'] ?? 'pending') ?></td>
